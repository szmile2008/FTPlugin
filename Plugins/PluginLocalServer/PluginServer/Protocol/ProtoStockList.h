...
class CProtoStockList: public CProtoParseBase
{
public:
	//tomodify 1 定义协议请求、协议响应结构体类型
	typedef StockList_Req		ProtoReqDataType;
	typedef StockList_Ack		ProtoAckDataType;

	//tomodify 2 定义协议请求、协议响应包体类型
	typedef StockListReqBody	ProtoReqBodyType;
	typedef StockListAckBody	ProtoAckBodyType;


public:
	CProtoStockList();
	virtual ~CProtoStockList();
