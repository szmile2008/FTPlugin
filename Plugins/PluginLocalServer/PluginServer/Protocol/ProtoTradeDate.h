...
class CProtoTradeDate: public CProtoParseBase
{
public:
	//tomodify 1 定义协议请求、协议响应结构体类型
	typedef TradeDate_Req		ProtoReqDataType;
	typedef TradeDate_Ack		ProtoAckDataType;

	//tomodify 2 定义协议请求、协议响应包体类型
	typedef TradeDateReqBody	ProtoReqBodyType;
	typedef TradeDateAckBody	ProtoAckBodyType;


public:
	CProtoTradeDate();
	virtual ~CProtoTradeDate();
