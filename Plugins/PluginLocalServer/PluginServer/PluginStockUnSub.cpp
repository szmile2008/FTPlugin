...
void CPluginStockUnSub::SetQuoteReqData(int nCmdID, const Json::Value &jsnVal, SOCKET sock)
{
	...
	VT_STOCK_DATA_REQ &vtReq = m_mapReqInfo[std::make_pair<INT64, int>(nStockID, req.body.nStockSubType)];
	...
	if ( bNeedSub )
	{
		...
		if ( bIsSub )
		{
			...
			if ( SubResult == StockSub_UnSubTimeError )
			{
				...
				MAP_STOCK_DATA_REQ::iterator it_iterator = m_mapReqInfo.find(std::make_pair<INT64, int>(nStockID, req.body.nStockSubType));
		...
		QuoteAckDataBody &ack = m_mapCacheData[std::make_pair<INT64, int>(nStockID, req.body.nStockSubType)];
	}
...
void CPluginStockUnSub::ClearQuoteDataCache()
{
	...
	for ( ; it_todel != m_mapCacheToDel.end(); )
	{
		...
		MAP_STOCK_DATA_REQ::iterator it_req = m_mapReqInfo.find(std::make_pair<INT64, int>(nStockID, nStockSubType));
		...
		else
		{
			...
			{
				m_mapCacheData.erase(std::make_pair<INT64, int>(nStockID, nStockSubType));
				it_todel = m_mapCacheToDel.erase(it_todel);
...
void CPluginStockUnSub::ReplyAllReadyReq()
{
	...
	for ( ; it_stock != m_mapReqInfo.end(); )
	{
		...
		int nStockSubType = it_stock->first.second;
		VT_STOCK_DATA_REQ &vtReq = it_stock->second;
		MAP_STOCK_CACHE_DATA::iterator it_data = m_mapCacheData.find(std::make_pair<INT64, int>(nStockID, nStockSubType));
		...
		it_stock = m_mapReqInfo.erase(it_stock);
		m_mapCacheToDel[std::make_pair<INT64, int>(nStockID, nStockSubType)] = dwTickNow;
